...
class Ligand:
    ...
    def __init__(self, ligand, ligand_name, working_dir="./", id=None):
        ...
        if not os.path.exists(self.mol2_file):
            ...
            out = AmberUtility.antechamber(self.prefix, self.prefix, self.name, nc=ch, grms_tol=0.05, scfconv="1.d-6",
                                     ndiis_attempts=700)
            if not os.path.exists(self.mol2_file):
                print(out)
...
class Complex:
    ...
    def __init__(self, protein: Protein, ligand: Ligand, complex_name, working_dir="./", domain_selection=None):
        ...
        if os.environ.get("AUTODOCK_GPU") is not None:
            with open('script.com', 'w') as script:
                script.write('$pythonMGL {}prepare_gpf4.py -l {}.pdbqt -r {}}.pdbqt -o {}.gpf \n'.format(
                    self.path_mlg, ligand.prefix, protein.prefix, protein.prefix))
                script.write('autogrid4 -p {}.gpf\n'.format(protein.prefix))
                script.write('${} -ffile {}.maps.fld -lfile {}.pdbqt -nrun 100\n'.format(
                    os.environ.get("AUTODOCK_GPU"), protein.prefix, ligand.prefix))
        else:
