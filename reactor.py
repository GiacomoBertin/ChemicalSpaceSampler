...
class Reactor(object):
    ...
    def __init__(self, rxn_library_variables, list_of_already_made_smiles, Filter_Object_Dict):
        ...
        self.reaction_dict = self.retrieve_reaction_dict(Rxn_library,
                                                         rxn_library_file)  # Retrieve the dictionary containing all the possible ClickChem Reactions
        ...
        self.functional_group_dict = {k: self.functional_group_dict[k][2:len(self.functional_group_dict[k]) - 2] for k
                                      in self.functional_group_dict.keys()}
    ...
    def retrieve_reaction_dict(self, Rxn_library, rxn_library_file):
        ...
        if rxn_library_file == "":
            ...
            if Rxn_library == "ClickChem":
                ...
                rxn_library_file = os.path.join(pwd, "Reaction_libraries", "Robust_Rxns",
                ...
                if os.path.exists(rxn_library_file) == False:
                    raise Exception("CUSTOM rxn_library_file cannot be found. Please check the path: ",
                                    rxn_library_file)
            else:
    ...
    def retrieve_functional_group_dict(self, Rxn_library, function_group_library):
        ...
        if function_group_library == "":
            ...
            if Rxn_library == "ClickChem":
                function_group_library = os.path.join(pwd, "Reaction_libraries", "ClickChem",
                                                      "ClickChem_functional_groups.json")
            elif Rxn_library == "Robust_Rxns":
                function_group_library = os.path.join(pwd, "Reaction_libraries", "Robust_Rxns",
                                                      "Robust_Rxns_functional_groups.json")
            elif Rxn_library == "All_Rxns":
                function_group_library = os.path.join(pwd, "Reaction_libraries", "All_Rxns",
                                                      "All_Rxns_functional_groups.json")
            elif Rxn_library == "CUSTOM":
                if os.path.exists(function_group_library) == False:
                    raise Exception("CUSTOM function_group_library cannot be found. Please check the path: ",
    ...
    def retrieve_complimentary_dictionary(self, Rxn_library, Complimentary_mol_dir):
        ...
        if Complimentary_mol_dir == "":
            if Rxn_library == "ClickChem":
                Complimentary_mol_dir = os.path.join(script_dir, "Reaction_libraries", "ClickChem",
                                                     "complimentary_mol_dir")
            elif Rxn_library == "Robust_Rxns":
                Complimentary_mol_dir = os.path.join(script_dir, "Reaction_libraries", "Robust_Rxns",
                                                     "complimentary_mol_dir")
            elif Rxn_library == "All_Rxns":
                Complimentary_mol_dir = os.path.join(script_dir, "Reaction_libraries", "All_Rxns",
                                                     "complimentary_mol_dir")
            elif Rxn_library == "CUSTOM":
                if os.path.isdir(Complimentary_mol_dir) == False:
                    raise Exception("CUSTOM Complimentary_mol_dir cannot be found. Please check the path: ",
                                    Complimentary_mol_dir)
            else:
                raise Exception("Rxn_library is not incorporated into SMILESCLICKCHEM.py")
        ...
        for group in functional_groups:
            ...
            if os.path.isfile(filepath) == True:
                ...
                print("Could not find the following .smi file for complimentary molecules for Mutation: {}".format(
                    filepath))

        if len(missing_smi_files) != 0:
            raise Exception("The following .smi file for complimentary molecules for Mutation is missing: ",
                            missing_smi_files)
    ...
    def determine_functional_groups_in_mol(self, mol_deprotanated, mol_reprotanated):
        ...
        for key in list(functional_group_dict.keys()):
            substructure = Chem.MolFromSmarts(str(functional_group_dict[key]))
    ...
    def run_Smile_Click(self, Ligand_smiles_string):
        ...
        try:
            mol = Chem.MolFromSmiles(Ligand_smiles_string,
                                     sanitize=False)  # This is the input molecule which serves as the parent molecule
        except:
        ...
        while tries < len(shuffled_reaction_list) and is_rxn_complete is False:
            ...
            #
                ...
                # Try to run reaction
                try:
                    reaction_product = rxn.RunReactants(tuple_reactant_mols)
                except:
                ...
                if reaction_product == () or reaction_product is None:
                    ...
                    reaction_product = reaction_product[0][0]
                    # Filter and check the product is valid
                    reaction_product_smilestring = self.check_if_product_is_good(reaction_product)
    ...
    def make_crossover(self, parent_smile_1, parent_smile_2, name_1, name_2):

        try:
            mol_1 = Chem.MolFromSmiles(parent_smile_1,
                                       sanitize=False)  # This is the input molecule which serves as the first parent molecule
            mol_2 = Chem.MolFromSmiles(parent_smile_2,
                                       sanitize=False)  # This is the input molecule which serves as the second parent molecule
        ...
        # try sanitizing, which is necessary later
        mol_1 = MOH.check_sanitization(mol_1)
        mol_2 = MOH.check_sanitization(mol_2)

        if mol_1 is None or mol_2 is None:
            return None

        # Is important for some functional groups while being deprotanated are useful for other reaction
        mol_1_reprotanated = copy.deepcopy(mol_1)
        mol_1_reprotanated = MOH.try_reprotanation(mol_1_reprotanated)
        mol_2_reprotanated = copy.deepcopy(mol_2)
        mol_2_reprotanated = MOH.try_reprotanation(mol_2_reprotanated)
        if mol_1_reprotanated is None or mol_2_reprotanated is None:
            return None

        mol_1_deprotanated = copy.deepcopy(mol_1)
        mol_1_deprotanated = MOH.try_deprotanation(mol_1_deprotanated)
        mol_2_deprotanated = copy.deepcopy(mol_2)
        mol_2_deprotanated = MOH.try_deprotanation(mol_2_deprotanated)
        if mol_1_deprotanated is None and mol_2_deprotanated is None:
            return None

        # Determine which functional groups are within a ligand
        list_subs_within_mol_1 = self.determine_functional_groups_in_mol(mol_1_deprotanated, mol_1_reprotanated)
        list_subs_within_mol_2 = self.determine_functional_groups_in_mol(mol_2_deprotanated, mol_2_reprotanated)

        if len(list_subs_within_mol_1) == 0:
            return None

        if len(list_subs_within_mol_2) == 0:
            return None
        ...
        while tries < len(shuffled_reaction_list) and is_rxn_complete is False:
            ...
            fun_groups_in_rxn = a_reaction_dict["functional_groups"]
            contains_group_mol1 = None
            contains_group_mol2 = None

            for i in range(0, len(fun_groups_in_rxn)):
                if fun_groups_in_rxn[i] in list_subs_within_mol_1:
                    contains_group_mol1 = i
                    # The number i which contains_group_mol1 is now equal to will be used to
                    # remember the placement of the molecule later in the reaction.
                    for j in range(0, len(fun_groups_in_rxn)):
                        if fun_groups_in_rxn[j] in list_subs_within_mol_2:
                            # The number j which contains_group_mol2 is now equal to will be used to
                            # remember the placement of the molecule later in the reaction.
                            contains_group_mol2 = j
                            break
                        else:
                            continue
                    break
            ...
            if contains_group_mol1 is None or contains_group_mol2 is None:
                # Reaction doesn't contain a functional group found in the reactant molecule.
                # So lets move on to the next molecule
                tries = tries + 1
                continue
                ...
                # Determine whether to react using the protanated or deprotanated form of the ligand
                substructure = Chem.MolFromSmarts(self.functional_group_dict[fun_groups_in_rxn[contains_group_mol1]])
                if mol_1_deprotanated.HasSubstructMatch(substructure) == True:
                    mol1_to_use = copy.deepcopy(mol_1_deprotanated)
                else:
                    mol1_to_use = copy.deepcopy(mol_1_reprotanated)
                substructure = None

                # Determine whether to react using the protanated or deprotanated form of the ligand
                substructure = Chem.MolFromSmarts(self.functional_group_dict[fun_groups_in_rxn[contains_group_mol2]])
                if mol_2_deprotanated.HasSubstructMatch(substructure) == True:
                    mol2_to_use = copy.deepcopy(mol_2_deprotanated)
                else:
                    mol2_to_use = copy.deepcopy(mol_2_reprotanated)
                substructure = None

            rxn = AllChem.ReactionFromSmarts(str(a_reaction_dict['reaction_string']))
            rxn.Initialize()

            list_reactant_mols = [mol1_to_use, mol2_to_use]
        ...
        if is_rxn_complete is True:
            ...
            product_info = [reaction_product_smilestring, reaction_id_number, name_1 + "_" + name_2]
            return product_info
        else:
            return None
